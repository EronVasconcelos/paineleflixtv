...
export default function App() {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (session) {
        // Se já carregamos uma vez (isFirstLoad = false), fazemos refresh silencioso
        fetchAllData(!isFirstLoad.current); 
      } else {
        // Logout
        setClients([]);
        setPackages([]);
  ...
  const fetchAllData = async (silent = false) => {
    ...
    try {
        ...
        if (isFirstLoad.current && !silent) setIsLoading(false);
            ...
            {view === 'clients' && (
              <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
                 <div className="flex flex-col md:flex-row gap-3">
                    <div className={`flex items-center px-3 py-2 rounded-md border flex-1 ${theme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
                 ...
                 <div className="grid gap-3">
                           ...
                           <div>
                              <div className="flex items-center gap-2">
                                 ...
                                 {client.paymentStatus === 'pending' && (
                                     <span className="bg-red-100 text-red-600 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase">Pagamento Pendente</span>
                                 )}
                              </div>
                   ...
                   {filteredClients.length === 0 && (
                       <div className="text-center py-10 text-slate-400 text-sm">Nenhum cliente encontrado.</div>
                   )}
                 </div>
              </div>
            )}
            ...
            {view === 'add' && (
               <div className={`max-w-2xl mx-auto p-6 rounded-xl border shadow-sm animate-in zoom-in-95 duration-300 ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                  <h2 className="text-lg font-bold mb-6 flex items-center gap-2">
                      <UserPlus size={20} className="text-blue-500"/> Novo Cliente
                  </h2>
                  <form onSubmit={handleAddClient} className="space-y-4">
                           ...
                           onChange={(e) => {
                               ...
                               if(pkg) {
                                   setAddFormData({
                                       ...
                                       expenses: pkg.cost.toString()
                                   });
                               } else {
                                   setAddFormData({...addFormData, packageId: e.target.value});
                               }
                           }}
                        >
                           <option value="">Personalizado</option>
                     ...
                     <div className="flex items-center gap-2 py-2">
                        <button 
                           ...
                           className={`w-10 h-6 rounded-full p-1 transition-colors ${addFormData.isPaid ? 'bg-emerald-500' : 'bg-slate-300'}`}
                        >
                           <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${addFormData.isPaid ? 'translate-x-4' : 'translate-x-0'}`}></div>
                        </button>
                     ...
                     <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-bold uppercase text-xs tracking-wide shadow-lg mt-4 transition-all">
                        Cadastrar Cliente
                     </button>
                  </form>
               </div>
                ...
                <div className="space-y-6">
                    <div className={`p-5 rounded-xl border shadow-sm ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                        ...
                        <form onSubmit={(e) => {
                            ...
                            <button type="submit" className="h-[42px] bg-blue-600 text-white rounded-md font-bold uppercase text-xs">
                        ...
                        {packages.map(pkg => (
                            <div key={pkg.id} className={`flex items-center justify-between p-4 rounded-lg border ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                                <div>
                                    ...
                                    <div className="text-xs text-slate-500">R$ {pkg.price} • Custo: R$ {pkg.cost} • {pkg.months} mês(es)</div>
                                </div>
                                <div className="flex gap-2">
                ...
                <div className="space-y-6">
                    <div className={`p-5 rounded-xl border shadow-sm ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                         ...
                         <form onSubmit={(e) => {
                             ...
                             <textarea 
                                name="body" 
                                className={`w-full p-3 rounded-md border outline-none text-[13px] font-medium h-24 ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}
                                placeholder="Use {{nome}}, {{vencimento}}, {{valor}}..."
                                required
                             ></textarea>
                             <button type="submit" className="w-full bg-emerald-600 text-white py-2 rounded-md font-bold uppercase text-xs">
                                 Salvar Modelo
                             </button>
                         </form>
                    </div>
                        ...
                        {templates.map(t => (
                            <div key={t.id} className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <button onClick={() => handleDeleteTemplate(t.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={14}/></button>
                                </div>
                    ...
                    <div className="grid gap-3">
                        {servers.map(server => (
                            <div key={server.id} className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                                ...
                                <button onClick={() => setSelectedServerForCredit(server)} className="w-full py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded text-xs font-bold uppercase hover:bg-slate-200 dark:hover:bg-slate-700">
                                    Gerenciar Créditos
                                </button>
                            </div>
                        ))}
                        ...
                        <ModalOverlay theme={theme} onClose={() => setSelectedServerForCredit(null)}>
                            ...
                            <div className="p-5">
                                <form onSubmit={(e) => {
                                    ...
                                    <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded-md font-bold uppercase text-xs mt-2">
                                        Confirmar Compra
                                    </button>
                                </form>
                            </div>
